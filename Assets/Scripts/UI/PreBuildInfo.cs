...
public class PreBuildInfo : UIBehaviour
{
    ...
    {
        // DO NOT TOUCH, SOMEHOW WORKS, I HAVE NO IDEA HOW
        if(s == status.set)
        {
            float canvasHeight = c.renderingDisplaySize.y * c.scaleFactor;
            //print(canvasHeight);
            rectTransform.anchoredPosition = new(gridPos.x - c.renderingDisplaySize.x / 2, (canvasHeight / 13 + gridPos.z) + ( - canvasHeight / 2 + rectTransform.rect.height / 2));
