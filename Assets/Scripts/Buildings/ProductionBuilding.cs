        ...
        if (!stoped)
        {
            while (working.Count > 0)
            {
                ...
                while (currentTime < prodTime && working.Count > 0) // timer
                {
                    ...
                    if (build.selected)
                    {
                        GameObject.Find("Production_button").GetComponent<ProductionButton>().UpdateButtonState(currentTime, prodTime);
                    }
                }
                ...
                for (int j = 0; j < production.ammount.Length; j++) // adds production yields to storage 
                {
                    ...
                    {
                        transform.parent.parent.parent.Find("Canvas (UI)").Find("Research Tree").GetComponent<Research>().research_progress += production.ammount[j];
                    }
                }
