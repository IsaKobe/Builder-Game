...
public class ProductionButton : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler, IPointerDownHandler, IPointerUpHandler
{
    ...
    void Start()
    {
        transform.GetChild(0).GetComponent<Image>().fillAmount = 0.01f;
        transform.GetChild(1).GetComponent<Image>().color = new();
    }
    ...
    public void OnPointerUp(PointerEventData eventData)
    {
        ...
        bool stopped = pD.StopProduction();
        transform.GetChild(0).GetChild(0).GetChild(0).gameObject.SetActive(!stopped);
        transform.GetChild(0).GetChild(0).GetChild(1).gameObject.SetActive(stopped);
    }
}
