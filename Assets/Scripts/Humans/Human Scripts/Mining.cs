...
public class Mining : MonoBehaviour
{
    public IEnumerator DoDig()
    {
        ...
        h.grid.RemoveTiles(r); // destroyes the minned block
        h.GetComponent<JobQueue>().RemoveJob(gameObject.GetComponent<Human>().jData.ID); // removes job order
        gameObject.GetComponent<Human>().order.Add($"ROCK{gameObject.GetComponent<Human>().debugIndex-1}");
        Task t = gameObject.GetComponent<Human>().LookForNew(); // finds new Job
    }
}
