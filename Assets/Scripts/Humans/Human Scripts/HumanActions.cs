...
public static class HumanActions
{
    ...
    public static void Dig(Human h)
    {
        ...
        if(r.integrity <= 0)
        {
            ...
            {
                GameObject chunk = Resources.Load("Chunk") as GameObject;
                r.ChunkCreation(chunk.GetComponent<Chunk>());
                
            }
            Humans humans = GameObject.FindWithTag("Humans").GetComponent<Humans>();
            // destroys the mined block
            humans.grid.RemoveTiles(r);
            humans.GetComponent<JobQueue>().CancelJob(JobState.Digging, h.jData.interest); // removes job order
        }
    }
    /// <summary>
