...
public class JobFinder : MonoBehaviour
{
    ...
    Vector3Int start;
    List<Vector3Int> finish;

    bool search;
    int id;
    public async Task<jobData> FindPath(Vector3Int _start, List<jobData> _jobs, List<Human> _humans)
    {
        this.start = _start;
        finish = new List<Vector3Int>();
        ...
        foreach (jobData j in _jobs)
        {
            finish.Add(j.jobPos);
        }
        foreach (var f in finish)
        {
            if (Vector3.Distance(_start, f) == 1)
            {
                search = false;
    ...
    public async Task CheckMove(int j)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            Vector3Int checkVec = new Vector3Int();
            switch (i)
            {
                case 0:
                    checkVec = new Vector3Int(vec.x + 1, 0, vec.z);
                    break;
                case 1:
                    checkVec = new Vector3Int(vec.x - 1, 0, vec.z);
                    break;
                case 2:
                    checkVec = new Vector3Int(vec.x, 0, vec.z + 1);
                    break;
                case 3:
                    checkVec = new Vector3Int(vec.x, 0, vec.z - 1);
                    break;
            ...
            if (road.Contains(checkVec))
            {
                ...
                if (Visited == false)
                {
                    foreach(var f in finish)
                    {
                        if (Vector3.Distance(checkVec, f) == 1)
                        {
                            search = false;
