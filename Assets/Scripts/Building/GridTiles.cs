...
public class GridTiles : MasterClass
{
    ...
    public void Enter(Vector3Int vec, GameObject g)
    {
        ...
        switch (selMode)
        {
            case SelectionMode.nothing: // set highlight, if build set selectedObject
                c = highlight;
            ...
            case SelectionMode.cancel: // set highlight
                c = new Color(1, 0, 0, 0.5f);
                break;

            case SelectionMode.dig: // set highlight, and manage markedTiles
                c = Color.yellow;
                if (drag)
                {
                ...
                else if (toBeDigged.Contains(g.GetComponent<Rock>()))
                {
    ...
    public void BreakAction(GameObject g)
    {
        switch (selMode)
        {
            case SelectionMode.nothing:
        ...
        if (g != null)
        {
            Enter(ToInt(g.transform.position), g);
        }
    }
    ...
    public void PrepDig()
    {
        ...
        foreach (var worker in workers) // triggers on every free human
        {
            ...
            {
                worker.StopC();
                worker.StartCoroutine(worker.LookForNew()); // look for new jobs;
            }
        }
    }
