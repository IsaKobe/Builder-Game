...
public class GridTiles : MonoBehaviour
{
    ...
    /// clicked(selected)
    /// </summary>
    public ClickableObject clickedObject; // clicked
    /// <summary>
    /// mouse over
    /// </summary>
    ...
    public void RemoveTiles(Rock _r)
    {
        ...
        markedTiles.Remove(_r);
        //ops.SetTile(new(new Vector3(vec.x, vec.z, 0)), null);
        GameObject replacement = Instantiate(Resources.Load(_r.assetPath) as GameObject, new(vec.x, _r.transform.localPosition.y, vec.z), Quaternion.identity, transform); // creates a road on the place of tiles
        replacement.name = replacement.name.Replace("(Clone)", "");
        replacement.transform.parent = GameObject.Find(replacement.name + "s").transform;
        MyGrid.UpdateGrid(new(vec), replacement.GetComponent<Road>()? replacement.GetComponent<Road>() : replacement.GetComponent<Water>());
        Remove(_r);
        Destroy(_r.gameObject); // destroys object
    }
    void PrepDig()
    {
        drag = false;
