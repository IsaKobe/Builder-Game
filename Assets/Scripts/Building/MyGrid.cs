...
public static class MyGrid
{
    ...
    //------------------------Updating------------------------//
    ////////////////////////////////////////////////////////////
    public static void PlaceBuild(Building building)
    {
        if(building.id == -1)
            building.UniqueID();
    ...
    public static void RemoveBuilding(Building building)
    {
        ...
        for (int x = Mathf.CeilToInt(building.transform.position.x - size.x / 2); x < Mathf.CeilToInt(building.transform.position.x + size.x/2); x++)
        {
            ...
            {
                grid[x, z] = (GameObject.Instantiate(Resources.Load("Tile/Road"), new(x, 0, z), Quaternion.identity, building.transform.parent.parent.GetChild(1)) as GameObject).GetComponent<Road>();
            }
        }
